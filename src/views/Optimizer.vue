      ...
      <v-container>
        ...
        <v-row>
          ...
          <v-col cols="12" md="10">
            <v-textarea
              name="csvRankings"
              label="CSV of Student Rankings of Companies"
              v-model="csvRankings"
            ></v-textarea>
            <v-text-field
            ...
            <v-expansion-panels>
              ...
              <v-expansion-panel>
                ...
                <v-expansion-panel-content>
                  <ol>
                    <li
                      v-for="(payload, i) in history"
                      :key="i"
                    >
                      <v-btn
                        @click.stop="save(filename(payload, i), payload.assignmentsList)"
                        color="deep-purple"
                        icon
                        dark
                      >
...
export default Vue.extend({
  ...
  methods: {
    ...
    filename(payload, iteration) {
      return `Iteration${iteration + 1}-${Object.keys(payload.assignments).length}AssignedStudents-${payload.nonAssignedStudents.length}UnassignedStudents-${payload.conflicts.length}Conflicts-${Math.trunc(100*payload.avgSatisfaction)/100}AvgRanking.csv`;
    },
    save(filename, data) {
